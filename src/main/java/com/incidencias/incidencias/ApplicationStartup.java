...
public class ApplicationStartup implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private void seedData() {
        ...
        if (((ArrayList<TipoHardware>) tipoHardwareRepository.findAll()).size() == 0) {
        ...
        // AÑADIR ESTADOS:
        if (((ArrayList<Estado>) estadoRepository.findAll()).size() == 0) {
        ...
        // AÑADIR PERMISOS:
        if (((ArrayList<Permiso>) permisoRepository.findAll()).size() == 0) {
            ...
            Permiso impExp = new Permiso("IEDA", "Permiso para la inportación y exportación de datos.");
